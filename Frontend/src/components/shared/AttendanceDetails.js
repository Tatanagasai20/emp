    ...
    const fetchAttendanceData = async () => {
      try {
        ...
        if (!employeeId && currentUser?.email) {
          ...
          const findResponse = await fetch(`/api/employee/find-by-email/${encodeURIComponent(currentUser.email)}`);
          if (findResponse.ok) {
        ...
        const response = await fetch(`/api/employee/${employeeId}/attendance-details?month=${month}&year=${year}`);
