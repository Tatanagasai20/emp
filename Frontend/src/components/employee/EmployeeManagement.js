  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingEmployee) {
        ...
        const response = await fetch(`/api/employee/${editingEmployee.id}`, {
          method: 'PUT',
          headers: {
      ...
      alert(`Failed to save employee: ${error.message}. Please check if the backend server is running on /api`);
    }
  };
